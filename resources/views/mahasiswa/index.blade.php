        ...
        <form method="GET" action="{{ route('mahasiswa.index') }}" class="mb-3">
            <div class="input-group">
                <input type="text" name="katakunci" class="form-control" value="{{ request('katakunci') }}"
                    placeholder="Cari mahasiswa, jurusan, atau matakuliah...">
                @if (request('katakunci'))
                    <a href="{{ route('mahasiswa.index') }}" class="input-group-text">
...
@endif
            ...
            <tbody>
                @foreach ($mahasiswa as $mhs)
                    <tr>
                        ...
                        <td>{{ optional ($mhs->jurusan)->nama_jurusan }}</td>
                        <td>{{ optional ($mhs->matakuliah)->nama_matakuliah }}</td>
                        <td>
                            ...
                            <form action="{{ route('mahasiswa.destroy', $mhs->id) }}" method="POST" class="d-inline"
                                id="delete-form-{{ $mhs->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#confirmDeleteModal{{ $mhs->id }}">Hapus</button>
                            </form>
                        </td>
                    </tr>
                    <div class="modal fade" id="confirmDeleteModal{{ $mhs->id }}" tabindex="-1"
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="button" class="btn btn-danger"
                                        onclick="document.getElementById('delete-form-{{ $mhs->id }}').submit();">Ya,
                                        Hapus</button>
                                </div>
                            </div>
                        </div>
        ...
        <div class="d-flex justify-content-end">
            {{ $mahasiswa->links('pagination::bootstrap-5') }} <!-- Pagination using Bootstrap 5 -->
        </div>
    </div>
