...
@endif
    ...
    <div class="input-group">
        <input type="text" name="katakunci" class="form-control" value="{{ request('katakunci') }}" placeholder="Cari matakuliah atau dosen...">
        @if(request('katakunci'))
            <a href="{{ route('matakuliah.index') }}" class="input-group-text">
...
@endif
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ optional($mk->dosen)->nama_dosen }}</td>
                    <td>
                        ...
                        <form action="{{ route('matakuliah.destroy', $mk->id) }}" method="POST" class="d-inline"
                                id="delete-form-{{ $mk->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                    data-bs-target="#confirmDeleteModal{{ $mk->id }}">Hapus</button>
                            </form>
                    </td>
                </tr>
                <div class="modal fade" id="confirmDeleteModal{{ $mk->id }}" tabindex="-1"
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="button" class="btn btn-danger"
                                        onclick="document.getElementById('delete-form-{{ $mk->id }}').submit();">Ya,
                                        Hapus</button>
                                </div>
                            </div>
                        </div>
        ...
        <div class="d-flex justify-content-end">
            {{ $matakuliah->links('pagination::bootstrap-5') }} <!-- Pagination using Bootstrap 5 -->
        </div>
    </div>
